"""
core/pathfinder/rf_train.py
Random Forest Model Trainer for ViperACL.
Trains, validates, and serializes the Balanced Random Forest classifier
using the RF enterprise telemetry dataset.
"""
...
def train_and_save_rf(verbose: bool = True) -> RandomForestClassifier:
    """Trains the enterprise Random Forest model and serializes the model artifacts."""
    train_path = PROJECT_ROOT / "data" / "rf_synthetic_training.csv"
    ...
    legacy_model_path = PROJECT_ROOT / "models" / "viper_rf_model.pkl"
    model_path.parent.mkdir(parents=True, exist_ok=True)

    if verbose:
        print("=" * 75)
        print("    VIPERACL PREDICTIVE ENGINE — RANDOM FOREST MODEL TRAINING")
        print("=" * 75)
    ...
    X_train = df_train[RF_FEATURE_COLUMNS]
    y_train = df_train["Success"]

    if verbose:
        print(f"[*] Ingested training dataset: {len(df_train)} samples across {len(RF_FEATURE_COLUMNS)} telemetry features.")
    ...
    # Save to both primary rf_viper_model.pkl and backwards-compatible path
    joblib.dump(rf_model, model_path)
    joblib.dump(rf_model, legacy_model_path)
    if verbose:
        print(f"[+] Model artifact serialized to: {model_path}")
        print("=" * 75)
