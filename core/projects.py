...
class ProjectManager:
    def __init__(self, projects_dir=None):
        ...
        self.projects_dir = projects_dir or os.path.join(base_dir, "projects")
    ...
    def list_projects(self):
        """Returns a list of all project metadata dictionaries."""
    ...
    def get_active_project_id(self):
        """Returns the currently active project_id (or None if no project is selected)."""
        data = self._load_data()
        active_id = data.get("active_project_id")
    ...
    def delete_project(self, project_id):
        ...
        if project_id in projects:
            ...
            if data.get("active_project_id") == project_id:
                data["active_project_id"] = list(projects.keys())[0] if projects else None
            self._save_data(data)
            return True
        return False
