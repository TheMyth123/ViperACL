...
Usage:
    ./venv/bin/python dev/purge.py [--yes]
"""
...
def purge_projects() -> int:
    """Resets data/projects/projects.json to pristine state and removes all project folders."""
    ...
    print(f"    [+] Reset projects.json (purged {cleared_count} registered records).")
    print(f"    [+] Removed {deleted_dirs} project workspace directories.")
    return cleared_count


def purge_logs() -> int:
    """Clears all session and audit log files in data/logs/."""
    ...
    # Recreate pristine empty audit log
    audit_file = logs_dir / "viperacl_audit.jsonl"
...
def purge_generated_scripts() -> int:
    """Removes generated remediation scripts from all standard directories."""
    ...
    # Ensure data/scripts/.gitkeep exists
    scripts_dir = PROJECT_ROOT / "data" / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
    gitkeep = scripts_dir / ".gitkeep"
    if not gitkeep.exists():
        gitkeep.write_text("# Keep data/scripts directory in git\n", encoding="utf-8")
...
def verify_preserved_assets():
    ...
    essential_assets = [
        ...
        ("Synthetic Training Dataset", PROJECT_ROOT / "data" / "synthetic_training.csv"),
        ("Synthetic Feature Descriptions", PROJECT_ROOT / "data" / "synthetic_desc.csv"),
        ("PathFinder ML Model", PROJECT_ROOT / "models" / "viper_rf_model.pkl"),
    ]
